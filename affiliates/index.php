...
$getweek = mysqli_query($con, "SELECT COUNT(c.id) AS `count` FROM counts c,links l where c.link_id= l.id and l.user_id=$user_id and WEEKOFYEAR(`date`) = WEEKOFYEAR(NOW())");
$resultgetweek = mysqli_fetch_assoc($getweek);

$getmonth = mysqli_query($con, "SELECT COUNT(c.id) AS `count` FROM counts c,links l where c.link_id= l.id and l.user_id=$user_id and YEAR(`date`) = YEAR(NOW()) AND MONTH(`date`) = MONTH(NOW())");
...
while($links = mysqli_fetch_assoc($getlinks)) {
	...
	$link=WEBSITE."/marketing/affiliates/get.php?id=" . $links["abbreviation"] ;
...
<div id="id01" class="modal">
 ...
 <script>
  ...
  var len= locations['lat'].length;
          ...
          $("#dialog").dialog({
                    modal: true,
                    title: "Showing approximate location of "+len+" click(s)",
                    width: "auto",
                    hright: "auto",
                    buttons: {
                        Close: function () {
